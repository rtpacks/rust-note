...
fn main() {
     ...
     *
     * > 在《unit 72-async 异步编程：Future 特征与任务调度》中详细的介绍了如何实现一个 Executor，复习后再来阅读本章内容。
     *
     * 在之前的章节中，已经实现过一版 Future 执行器（运行时），而在 rust 中 Future 与 async/.await 是非常重要的技能。
     * 为了更好的掌握 rust 开发，现在再次了解 Future 和 async/await。
     *
     * ### Future
     ...
     * 除了并发执行外，使用 Future 管理子 Future 也可以实现串行执行。并发执行和串行执行是最基础的使用 Future 特征表达异步控制的形式。
     *
     *
     * ### Waker 唤醒任务
     *
     ...
     *
     * rust 通过 Context 和 Waker 的组合，每个 Future 在注册其 wake 函数时，可以将自身的信息存储在 Waker 中。
     * 当 wake 被调用时，Future 的自身信息会被传递给执行器，从而使执行器能够正确识别并调度特定的 Future。
     *
     ...
     * 由于是在不同线程间共享状态，需要考虑所有权和并发状态，即需要使用 Arc 和 Mutex 两个工具。
     *
     *
     * **整体流程如下：**
     *
     ...
     * ```
     *
     * 以上代码可以描述一个 Future 的运行逻辑，但对于一个完整的 Future 运行，它还缺少三个步骤：
     * 1. Future 的创建
     ...
     * ```
     *
     * 最后改造 Channel 不是 sync 消息通道的问题，使用 `crossbeam` 库即可。
     *
     */
