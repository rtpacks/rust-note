...
fn main() {
     ...
     * ```
     *
     * 抽离单独文件后还需要关联原有的关系，让rust编译器知道抽离的模块文件属于当前模块的子模块。在 `src/lib.rs` 中用 `mod` 声明：
     * ```rs
     ...
     *
     * 因此在rust项目的代码中，不仅可以看到直接用 `mod` 声明模块的写法，也可以看到用 `mod` 加载同名模块文件的写法。
     *
     * #### 2. 拆分成文件夹
     *
     ...
     * 两者之间的最大区别在于：**文件夹不完全管理模块将所有的子模块放入与模块同名的文件夹中管理，而模块文件是放在与父模块同级的层级下。**
     *
     *
     *
     *
