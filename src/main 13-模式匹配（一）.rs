fn main() {
    ...
    /*
     * ## 模式匹配
     * Rust中经常使用到的一个强大功能是模式匹配(pattern match)，例如let变量赋值本质上就是在进行模式匹配。
     * 得益于Rust模式匹配功能的强大，使用模式匹配比不使用模式匹配，往往会减少很多代码。
     *
     * 一句话描述模式：模式是一个带有类型变量，可以比较类型的同时也可以比较值
     *
     * 可在如下几种情况下使用模式匹配：
     * - let变量赋值
     * - 函数参数传值时的模式匹配
     ...
     * ```
     *
     * => 左边的是各分支的模式，模式是一个带有类型变量，有些带有具体值的是字面量类型，表示这个当前位置只能是字面量类型标识的值。如（12，n）表示元组第一个是12类型，值只能是12。
     * VALUE将与这些分支逐一进行匹配，=>右边的是各分支匹配成功后执行的代码。每个分支后使用逗号分隔各分支，最后一个分支的结尾逗号可以省略(但建议加上)。
     *
