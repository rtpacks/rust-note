...
fn main() {
     ...
     *
     * 在rust中，变量的所有权是第一个值得关注的点，所以在模式解构中，为了避免进行解构赋值时，
     ...
     * - 方式三：在模式的某些字段或元素上使用ref关键字修饰变量，如果希望变量可变，则使用mut关键字。
     *
     * ```rs
     ...
     * let ref x = &5_i32;    // x的类型：&&i32
     * ```
     * **在模式中使用ref修饰变量名相当于对被解构的字段或元素上使用 `&` 进行引用，ref就是定义变量是否为引用类型。**
     * 因此，使用ref修饰了模式中的变量后，解构赋值时对应值的所有权就不会发生转移，而是以**只读**的方式借用给该变量。
     *
     * 如果想要对解构赋值的变量具有数据的修改权，需要使用mut关键字修饰模式中的变量，但这样会转移原值的所有权，此时可不要求原变量是可变的。
     * 如果不想在可修改数据时丢失所有权，可在mut的基础上加上ref关键字，就像&mut xxx一样。
     *
     * ref与mut和&与mut的组合是相同的，只是模式解构的pattern位置不用&而用ref表达属性的引用。
     *
     * 注意，使用ref修饰变量只是借用了被解构表达式的一部分值（属性），而不是借用整个值。如果要匹配的是一个引用，则使用&表示。
     * ```rs
     * let a = &(1,2,3);       // a是一个引用
     ...
     * 其实rust的模式解构不需要我们去考虑实现方式是什么，只需要我们比对需要的数据格式是什么，这是为了方便使用所有权而设计的。
     *
     * 比如
     ...
     *
     *
     * 最后，也可以将 match value{} 的value进行修饰，例如match &mut value {}，这样就不需要在模式中去加ref和mut了。
     * 这对于有多个分支需要解构赋值，且每个模式中都需要ref/mut修饰变量的match非常有用。
     ...
     * - 如果t不是一个引用，则t的每一个元素都会move给i（所有权转移）
     * - 如果t是一个引用，则i将是每一个元素的引用
     * - 同理，for i in &mut t和for i in mut t也一样
     *
     * ### 解构解引用类型
     * 当match VALUE的VALUE是一个解引用*xyz时(因此，xyz是一个引用)，可能会发生所有权的转移，
     * 此时可使用xyz或&*xyz来代替*xyz。具体原因请参考：[解引用(deref)的所有权转移问题](https://rust-book.junmajinlong.com/ch6/05_re_understand_move.html#whenmove)。
     *
     * ```rs
     * fn main() {
     *   // p是一个Person实例的引用
