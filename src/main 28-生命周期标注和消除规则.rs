...
fn main() {
     ...
     * 生命周期标注以 `'` 开头，名称往往是一个单独的小写字母，大多数人都用 `'a` 来作为生命周期的名称。如果是引用类型的参数，那么生命周期会位于引用符号 & 之后，并用一个空格来将生命周期和引用参数分隔开。
     * 为什么特别指引用类型呢？因为非引用类型数据的所有权都在自身上，不会出现悬垂引用的问题。同时要特别注意的是：**生命周期标注只是标注，只是帮助编译器推理生命周期，一些错误的标注编译器是不会接受，会报错的。**
     *
     * ```rs
     * &i32        // 一个引用
     * &'a i32     // 具有显式生命周期的引用
     ...
     * 实际上，这意味着**返回值的生命周期与参数生命周期中的较小值一致**。虽然两个参数的生命周期都是标注了 'a，但是实际上这两个参数的真实生命周期可能是不一样的(生命周期 'a 不代表生命周期等于 'a，而是大于等于 'a)。
     *
     * #### 深入思考生命周期标注
     ...
     * - 方法签名中，往往不需要标注生命周期，得益于生命周期消除的第一和第三规则（存在多个输入生命周期，其中一个为&self或者&mut self，则&self的生命周期被赋给所有的输出生命周期）。
     *
     * 第三规则会给所有的输出生命周期赋予&self的生命周期，那么需要自定义生命周期呢？这需要满足一定的条件，首先是业务代码：
     * ```rs
     * struct ImportantExcerpt<'a> { part: &'a str };
     *
     ...
     *
     * 回到生命名周期中，我们提到过一句话，**保证依赖有效**，只需要保证自定义的输出生命周期内，&self生命周期是有效的就能保证依赖有效。
     * 换句话说：&self生命周期包含自定义的生命周期。这个道理很简单，`self.part` 是part依赖于self，self的生命周期比part大才不会出现悬垂引用问题。
     *
     * ### 5. 静态生命周期
     ...
     * - 字符串字面量是'static的生命周期，在分析生命周期时需要明确标出字符串字面量，否则容易混淆
     *
     *
     * ### 一个复杂的例子
     * ```rs
