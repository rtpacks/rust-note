fn main() {
     ...
     *
     * Trait 定义了一组可以被共享的行为，只要实现了特征，你就能使用这组行为
     *
     * ### Trait 和 interface
     *
     ...
     * - Trait类型：用于定义抽象行为，抽取那些共性的属性，主要表现是作为泛型的数据类型(对泛型进行限制)，类似 `String` 作为类型限制数据类型。
     * - Trait对象：即Trait Object，能用于多态。类似其他语言中的接口（是实现接口，不是继承父类，没有继承的概念，Rust更强调组合）。组合是灵活且易读的，当继承链非常长时，难以判断属性是继承于谁
     *
