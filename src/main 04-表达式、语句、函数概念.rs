fn main() {
     ...
     *
     * ## 函数
     * 函数的返回值在函数体内可以由两种形式表达，第一是表达式，第二是return关键字。
     ...
     *      a + b + 5
     * }
     * 在if表达式内部的a+b-5表达式，就不属于函数的返回值表达式。只有当表达式处于函数体最后一行时才是函数的返回值。
     * 关键字return就提供在函数体内任意位置设置函数返回值的能力。
     *
     * ### 注意
     * 函数分为收敛函数和发散函数，即函数的返回值可以分为有返回值和无返回值两类，返回唯一类型也代表着有返回值。
     * 无返回值可以视为never，如报错后/死循环函数无法正常结束，为无返回值，用!表示。
    ...
    fn add_a(a: i32, b: i32) -> i32 {
        a + b
    }

    // 函数和表达式
    fn opr_five(a: i32, b: i32) -> i32 {
        let c = if a > 4 { a + b } else { 1 };
