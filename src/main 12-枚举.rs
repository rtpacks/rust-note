...
fn main() {
     ...
     * 例如【整数】虽然包含0、1、2、......，但这样的值无穷无尽，此时不应该直接用枚举类型，而应该使用具有概括性的方式去描述它们，比如枚举正整数、0、负整数这三种情况，也可以枚举所需的1、2、3后，再用一个额外的Other来通配所有其他情况。
     * Rust支持枚举类型，且Rust的枚举类型比其他语言的枚举类型更为强大。
     *
     * 任何类型的数据都可以放入枚举成员中: 例如字符串、数值、结构体甚至另一个枚举。
     *
     * Rust的结构体更像是面向对象语言中的接口或父类，它能统一某一种类型的变量，不通过继承做到多态。现代语言推崇使用组合而不是继承。
     *
     * Java中，多态的实现是通过实现接口或继承父类来实现的，达到复用的目的。
     * ```java
     * class Message {
     *      String message;
     * }
     ...
     * ```
     *
     * 在Rust中，没有继承这个概念，Rust推崇使用组合！如以下代码。
     * ```rs
     * enum Message {
     *     Quit,
     *     Move { x: i32, y: i32 },
     ...
     *
     * Rust中结构体是没有继承这个概念的，所以在实现不同的类型需要多个结构体
     * ```rs
     ...
     * }
     * ```
     * 注意：可在enum定义枚举类型的前面使用#[repr]来指定枚举成员的数值范围，超出范围后将编译错误。当不指定类型限制时，Rust尽量以可容纳数据大小的最小类型。例如，最大成员值为100，则用一个字节的u8类型，最大成员值为500，则用两个字节的u16。
     * ```rs
     * // 最大数值不能超过255
     * #[repr(u8)]  // 限定范围为`0..=255`
     *   enum E {
