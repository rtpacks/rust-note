...
fn main() {
     ...
     * ```
     *
     * 也就是说，迭代器可以使用 `into_iter` 方法，能形成以下代码，**迭代器能够调用into_iter方法，返回的还是迭代器**：
     * ```rust
     * let v = vec![1,2,3];
     ...
     * - into_iter 会夺走所有权，next函数返回的是Option<T>，即Some<T>和None
     * - iter 是不可变借用，next函数返回的是Option<&T>，即Some<&T>和None
     * - iter_mut 是可变借用，next函数返回的是Option<&mut T>，即Some<&mut T>和None
     *
     * rust方法的命名一般都遵守这个规则：**into_ 之类的都是拿走所有权，_mut 之类的都是可变借用，剩下的就是不可变借用**
     *
     * ```rust
     * let v = vec![1, 2, 3];
     ...
     * Iterator和IntoIterator两者与JavaScript迭代器中的迭代器协议和可迭代协议是类似的。
     *
