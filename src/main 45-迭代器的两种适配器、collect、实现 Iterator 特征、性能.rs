...
use std::{array::IntoIter, collections::HashMap, env, error::Error, fmt::Display, fs, process};
...
fn main() {
     ...
     * 
     * > 疑问
     * > rust类型和值的关系非常巧妙，编译器可以通过变量的类型确定函数调用表达式的值，这属于什么原理。比如 let vector: Vec<_> = vec![1,2].into_iter().map(|x| x+1).collect(); 可以通过标注vector的类型让collect知道要收集什么数据。
     * > 这属于 Rust 的类型推断和类型系统的特性。Rust 的类型系统是静态的，意味着所有变量的类型都是在编译时确定的。Rust 编译器使用强大的类型推断算法来帮助开发者减少必须显式指定的类型数量，同时保持类型安全。
     * > 在你提到的例子中，let vector: Vec<_> = vec!.into_iter().map(|x| x+1).collect();，Rust 编译器利用类型推断来确定 _ 应该是什么类型。这里的 _ 是一个类型占位符，告诉编译器我们希望它自动推断这个位置的具体类型。编译器查看整个表达式，特别是 collect 方法的调用，来确定需要收集的数据类型。
